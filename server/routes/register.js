...
router.post('/', function(req, res, next) {
    ...
    var sql_command = 'select count(user_name) as num, user_name from account where user_name = ? and email = ?';
    query(sql_command, [req.body.user_name, req.body.email], function (err, result) {
        if(err){
        ...
        if(result[0].num > 0)
        {
            console.log('Register failed! The user name or email has already been registered.');
            res.send({
                'success': false,
                'msg': 'Register failed! The user name or email has already been registered.'
            });
            return;
        }
    });
